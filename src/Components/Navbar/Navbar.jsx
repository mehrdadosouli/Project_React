...
import { clearLocalStorage, getToken, scrollToTop } from '../../utils/funcs';
...
export default function Navbar({ text }) {
    ...
    return (
        <>
            <div className='flex justify-between items-start md:px-10 p-10 lg:bg-white bg-bg-primary-dark text-white lg:text-black lg:rounded-xl'>
                <div className={`flex lg:flex-row flex-col justify-start items-start gap-10 list-none`}>
                    ...
                    {
                        menu && menu.map(item =>
                            <li key={item.id} className={`group transition-colors relative ${showSubmenu && item.page == activeSubmenu ? 'bg-orange-200' : 'hover:bg-orange-200'} rounded-xl p-2 hover:cursor-pointer ${showMenu ? 'visible' : 'lg:flex hidden'} `}><Link to={item.link}> {item.page}</Link>
                                ...
                                {item.submenu ? <ul className={`md:min-w-96 min-w-72 h-fit p-5 text-white gap-10 bg-bg-primary-light z-50 transition-1 ${resizeScreen < 1025 ? showSubmenu && activeSubmenu == item.page ? 'flex flex-col' : 'hidden' : 'absolute top-20 rounded-xl invisible group-hover:visible'}`}>
                                    {item.submenu.map(sub =>
                                        <li key={sub.page} className='p-2 lg:my-5 transition-colors hover:bg-orange-200 rounded-xl cursor-pointer'><Link to={sub.link}>{sub.page}</Link></li>
                                    )}</ul>
                                    : ""
                                }
                            </li>
                ...
                <div className="left flex items-center md:gap-5">
                    {
                        !user ? <span className={`md:p-1 p-3 lg:text-black text-${text}`}><Link to="/login" >Login</Link></span> : (<Link to='/mypanel' className={`md:p-2 lg:text-black text-${text}`}>پنل {user.name}</Link>)
                    }
                    {
                        user && <Link onClick={logOutHandler} to='/login' className={`p-3 rounded-lg ml-3 lg:text-black text-${text}`}>LogOut</Link>
                    }
                </div>
            </div>
        </>
