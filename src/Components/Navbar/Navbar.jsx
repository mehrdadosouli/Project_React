        ...
        <>
            <div className='flex justify-between items-start md:px-10 p-10'>
                <div className={`flex lg:flex-row flex-col justify-start items-start gap-10 list-none`}>
                    ...
                    {
                    menu && menu.map(item => 
                        <li className={`group relative transition-colors hover:bg-orange-200 rounded-xl p-2 hover:cursor-pointer ${showMenu ? 'visible' : 'lg:flex hidden'} `}><Link to={item.link}> {item.page}</Link>
                        { item.submenu && window.innerWidth < 757 && (<span className={`mr-5 p-2 `} onClick={()=>showSubmenuHandler(item)} > {">"} </span>)}
                            {item.submenu ? <ul className={`md:min-w-96 min-w-72 h-fit p-5 flex flex-col text-white gap-10 bg-green-700 z-50 absolute md:top-20 top-2 md:right-0 right-40 transition-1 md:group-hover:visible md:group-hover:opacity-100 opacity-0 ${showSubmenu ? 'group-hover:visible group-hover:opacity-100 ' : 'invisible'}`}>
