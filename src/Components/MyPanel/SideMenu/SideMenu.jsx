...
function SideMenu() {
  ...
  return (
      <div className='sm:static fixed top-0 right-0 p-10 flex flex-col z-50'>
        <span className={`w-screen h-screen inline-flex bg-gray-600 opacity-55 absolute top-0 right-0 transform  ${showMenu ? 'translate-x-0' : 'translate-x-full' }`} onClick={closeHandler}></span>
          ...
          <div className={`w-72 flex flex-col gap-10 sm:visible translate-x-[1rem] sm:bg-inherit bg-slate-500 text-center sm:transform-none transform transition-all ease-linear ${showMenu ? 'visible translate-x-[1rem]' : 'invisible translate-x-[10rem]'}`}>
